...
fun 菜单栏() {
    //从配置文件中获取菜单项数据
    println( R.propertiesItem("menu_bar"))
    ...
    设置菜单栏 = Menu(settingData.get("item_name") as String)
    ...
    topBar.menus.addAll(menu,设置菜单栏)//将menu放到菜单栏当中
    root.top = topBar
    菜单栏操作()
}
...
private data class MenuBeanReader(val menu_name:MenuItemReader){
